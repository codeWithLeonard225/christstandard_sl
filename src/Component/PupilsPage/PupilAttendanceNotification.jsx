...
const IndividualReportCardTerm1 = () => {
  ...
  useEffect(() => {
    ...
    const loadAndListenGrades = async () => {
      ...
      const q = query(collection(db, "PupilGrades"), where("academicYear", "==", academicYear), where("schoolId", "==", schoolId), where("className", "==", selectedClass));
      const unsubscribe = onSnapshot(q, (snapshot) => {
  ...
  useEffect(() => {
    ...
    const loadAttendance = async () => {
      ...
      const q = query(collection(db, "PupilAttendance"), where("studentID", "==", selectedPupil), where("schoolId", "==", schoolId), where("academicYear", "==", academicYear));
      const unsubscribe = onSnapshot(q, (snapshot) => {
