...
const TeacherGradesPage = () => {
  ...
  // 2. Bulk Fetch and Cache ALL Teacher's Pupils 
  useEffect(() => {

    if (!academicYear || assignments.length === 0) return;

    // teacherClasses are now guaranteed to be clean/trimmed
    const teacherClasses = assignments.map(a => a.className);

    const q = query(
      collection(db, "PupilsReg"),
      where("class", "in", teacherClasses), // This query now uses the clean class names
    ...
    const unsub = onSnapshot(q, async (snapshot) => {
      ...
      if (selectedClass) {
        // This local filter now works because both sides (p.class and selectedClass) are trimmed
  ...
  const handleSubmitGrades = async () => {
    ...
    try {
      ...
      // ⭐ FIX 2: Ensure the class name is trimmed before submission
      ...
      for (const pupilID of Object.keys(currentGrades)) {
        ...
        if (gradeValue !== "") {
          ...
          await setDoc(docRef, {
            pupilID,
            className: trimmedClass, // Use the guaranteed clean class name
